...
public class Ex02_Arrays {
	public static void main(String[] args) {
		...
		//배열에 정의된 값을 "문자열 형태"로 변환하여 출력해준다.
		...
		//int -> Integer (int의 클래스타입 : 객체)
		
		Integer[] iArr = {1,6,2,3,10,7,4,5,8,9}; //객체
		...
		//서로 간의 값이 함께 변경된다. (주소를 복사한다)
		
		//깊은 복사(Deep Copy) : 복사된 배열이나 원본 배열이 변경될 때
		//서로 간의 값은 바뀌지 않는다. (값을 복사한다) 
		...
		//얕은 복사는 주소가 복사되기 때문에 값을 변경시 복사본과 원본의 값이 같이 바뀐다.
		
		//깊은 복사
		//배열의 깊은 복사는 반복문을 이용해 새로운 배열에 값을 직접 넣어주거나
		//Arrays 클래스 또는 System 클래스가 가진 기능을 이용한다.
		...
		//깊은 복사는 값을 복사하기 때문에 복사 후 복사본의 값을 바꿔도 원본과 다른 복사본은
		//값이 변하지 않는다.
		
		//다차원 배열
		//2차원 이상의 배열을 의미하며, 배열의 요소로 또 다른 배열을 가지는 것을 의미한다.
		//2차원 배열은 요소로써 1차원 배열을 가진다.(배열의 중첩)
